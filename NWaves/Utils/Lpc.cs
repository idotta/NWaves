...
public static class Lpc
{
    ...
    public static void FromLsf(float[] lsf, float[] lpc)
    {
        ...
        if (n % 2 == 1)
        {
            ps = Operation.Convolve(ps, new ComplexDiscreteSignal(1, [1.0, 0, -1.0]));
        }
        else
        {
            ps = Operation.Convolve(ps, new ComplexDiscreteSignal(1, [1.0, -1.0]));
            qs = Operation.Convolve(qs, new ComplexDiscreteSignal(1, [1.0,  1.0]));
        }
